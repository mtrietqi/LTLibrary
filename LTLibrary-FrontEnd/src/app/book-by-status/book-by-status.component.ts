...
@Component({
  selector: 'app-book-by-status',
  templateUrl: './book-by-status.component.html',
  styleUrls: ['./book-by-status.component.css'],
  providers: [CookieService]
})
export class BookByStatusComponent implements OnInit {
  //Server link
  ...
  getBookById(id: string) {
    // alert(id);
    this.http.get(this.serverLink+'/getAllBookByStatusREST/' + id).map((res: Response) => res.json())
      .subscribe(dataEdit => {
  ...
  showBook() {
    this.http.get(this.serverLink+'/getAllBookByStatusREST/' + $("#statusSelectionEdit").val()).map((res: Response) => res.json())
      .subscribe(data => {
        this.data = data;
        // console.log(this.dataEdit);
      });
